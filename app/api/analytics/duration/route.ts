...
export async function POST(req: NextRequest) {
  let body: { id?: number; visitorId?: string; durationMs?: number }
  try {
    body = await req.json()
  } catch {
  ...
  const { id, visitorId, durationMs } = body
  if (
    typeof id !== 'number' || !Number.isFinite(id)
    || typeof visitorId !== 'string' || !visitorId
    || typeof durationMs !== 'number' || !Number.isFinite(durationMs) || durationMs < 0
  ) {
    return NextResponse.json({ error: 'id, visitorId, and durationMs are required' }, { status: 400 })
  }
