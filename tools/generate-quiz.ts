...
const { data: topic, error } = await supabase
  .from('topics')
  .select('id,title,answer,answer_full,objections,scripture,catechism,difficulty')
  .eq('id', topicId)
  .eq('lang', lang)
  .maybeSingle()
...
// `answer` is a plain string for the Apologetics essay topics (no `answer_full`), but an
// {summary, full} object for topics generated by the qwen pipeline — handle both.
const answerText = typeof topic.answer === 'string' ? topic.answer : (topic.answer as { summary?: string } | null)?.summary
const objectionsText = Array.isArray(topic.objections) && topic.objections.length > 0
  ? '\n\n' + (topic.objections as { objection: string; response: string }[])
      .map(({ objection, response }) => `Q: ${objection}\nA: ${response}`)
      .join('\n\n')
  : ''
const sourceText = (topic.answer_full || answerText || '') + objectionsText
if (!sourceText.trim()) throw new Error(`Topic ${topicId} has no answer_full, answer text, or objections to ground on`)
...
/** Returns the question text of any item that doesn't have exactly 4 choices. */
function findWrongChoiceCountQuestions(qs: QuizQuestion[]): string[] {
  return qs.filter((q) => q.choices.length !== 4).map((q) => q.question)
}
...
while (accepted.length < count && chunkRound < MAX_CHUNK_ROUNDS) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS_PER_CHUNK; attempt++) {
    ...
    const wrongCount = findWrongChoiceCountQuestions(chunkQuestions)
    const dupes = findDuplicateChoiceQuestions(chunkQuestions)
    if (wrongCount.length === 0 && dupes.length === 0) break
    if (wrongCount.length > 0) {
      console.log(`  [round ${chunkRound}, attempt ${attempt}] ${wrongCount.length} question(s) didn't have exactly 4 choices, retrying`)
      extraNote = `The previous attempt had questions without exactly 4 choices — regenerate the whole batch and give every question exactly 4 choices: ${wrongCount.join(' | ')}`
    } else {
